...
<Steps>
  ...
  <Step title="Create a Service ID">
    ...
    6. Enter a unique identifier, which could be your app ID with `.signin` appended (e.g. `com.yourcompany.yourapp.signin`)
  ...
  <Step title="Create a Private Key">
    ...
    6. Select your primary App ID (e.g., `com.yourcompany.yourapp`) and click `Save`
...
const CLIENT_ID = "com.example.service";      // your Service ID
...
3. Edit the four const lines at the top of `gen_apple_secret.js`:
   ...
   - `CLIENT_ID`: The Service ID you created (e.g. `com.yourcompany.yourapp.signin`)
...
<Steps>
  <Step title="Configure Supabase Auth Provider">
    ...
    4. Fill in the required fields:
       - **Client IDs**: The Service ID you created (e.g., `com.yourcompany.yourapp.signin`)
...
1. Rebuild and run your app using a development build (Apple Sign-In doesn't work in Expo Go)
